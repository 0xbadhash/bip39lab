...
test.describe("Lab shell & chrome", () => {
  ...
  test("S100 three distinct Lab hover overlays (no OK)", async ({ page }) => {
    ...
    await page.locator("#btnDerive").hover();
    await expect(page.locator("#overlayDerive")).toBeVisible();
    await expect(page.locator("#overlayDeriveBody")).toContainText(/fills receive addresses/i);
    await expect(page.locator("#overlayDerive")).toContainText(/not a wallet/i);
    await expect(page.locator("#overlayDerive .lab-overlay-ok")).toHaveCount(0);
    await page.locator("#btnClear").hover();
    await expect(page.locator("#overlayClear")).toBeVisible();
    await expect(page.locator("#overlayClearBody")).toContainText(/Empties this tab only/i);
    await expect(page.locator("#overlayClear .lab-overlay-ok")).toHaveCount(0);

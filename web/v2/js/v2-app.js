...
(function () {
  ...
  var TRACKS = [
    ...
    { id: 21, level: "Intermediate", title: "Collaborative custody", job: "You hold two keys. A company holds one. They can co-sign. They cannot steal. They cannot freeze you if you still have both keys.", done: "2-of-3: you can spend alone with your two keys. One company key cannot steal or freeze that spend. They can refuse to help if you lost a key." },
    ...
    { id: 28, level: "Advanced", title: "CoinJoin / privacy", job: "A leftover piece (UTXO) spent with another looks like one owner. A joint spend obfuscates who paid. It is not a backup.", done: "You combined two pieces and saw the link. You saw equal outputs hide which is yours. Mixing is not custody. This tab never signs." },
  ...
  var GATES = {
    ...
    28: { is: "UTXOs are leftover pieces. Combining them in one spend can link you. A joint spend with equal outputs obfuscates who paid whom. The chain still lists everything.", isnt: "Not a mixer brand. Not a backup. This tab never signs or broadcasts." },
  ...
  function stepsFor(id) {
    var map = {
      ...
      28: ["What is a leftover piece", "Combining links you", "Equal outputs obfuscate", "Quiz", "Finish"],
      29: ["Two vaults, same words", "PIN vs wipe", "Chain still has the coins", "Quiz", "Finish"],
  ...
  function conceptsFor(id) {
    var c = {
      ...
      28: ["UTXO leftover", "Two inputs → one owner guess", "Joint equal outputs"],
      29: ["Empty vs extra secret", "Two PINs / wipe PIN", "Coins live on the chain"],
  ...
  var VIZ = {
    ...
    28: {
      forStep: function (s) { if (s <= 0) return 1; if (s <= 1) return 2; return 3; },
      atoms: [
        atom(1, 0, "assets/uc4-atom-path-folder.svg", "A leftover piece is a UTXO", "<strong>Plan · Pieces</strong><br/>Your total is leftover coins from earlier payments, not one bank number. UC27 spent one piece. This track asks what happens if you spend two together."),
        atom(2, 1, "assets/uc9-atom-leaks-history.svg", "Two inputs, one guess", "<strong>Practice · Combine</strong><br/>Spend cafe leftover + salary leftover in one payment. An outsider often guesses one owner."),
        atom(3, 2, "assets/uc9-atom-watch-only.svg", "Equal outputs obfuscate", "<strong>Review · Joint spend</strong><br/>Many people, same-size outputs. Who paid the merchant is harder to tell. Still on the chain. Not a backup. No brand.")
      ]
    },
    29: {
      forStep: function (s) { if (s <= 0) return 1; if (s <= 1) return 2; return 3; },
      atoms: [
        atom(1, 0, "assets/uc3-atom-same-words.svg", "Same words, two extras, two vaults", "<strong>Plan · Two mailboxes</strong><br/>Empty extra vs a practice extra. Both addresses are real. Practice amounts only."),
        atom(2, 1, "assets/uc12-atom-hardware.svg", "Two PINs or a wipe PIN", "<strong>Practice · Device</strong><br/>Ledger: two PINs can open two extras. Coldcard: a wipe PIN erases the chip, not the chain."),
        atom(3, 2, "assets/uc3-atom-forgotten-loss.svg", "Coins live on the chain", "<strong>Review · Not on the box</strong><br/>Words + extra still open the vault after a wipe. This is not safety advice.")
      ]
    },
    30: {
      forStep: function (s) { if (s <= 0) return 1; if (s <= 2) return 2; return 3; },
  ...
  function entropyTeachHtml() {
    return teachBox(
      ...
      classLines(
        "Entropy is how hard the secret is to guess — how mixed the random bits are. " +
          inlineI(
            "entropy",
            "How random the secret is. More entropy = harder to guess. Not a second password and not a bitcoin address."
          ),
        "A 12-word list is 128 bits. 15 → 160, 18 → 192, 21 → 224, 24 → 256. Longer list, harder to guess.",
        "The orange meter below is <em>this</em> list — not a guess from the dropdown. 12 words → 128 bits, 24 words → 256. Practice only."
      ),
      "v2EntropyWhat"
    );
  }
  ...
  function entropyPhraseBits(phrase) {
    var m = phrase != null ? phrase : mem.mnemonic;
    var n = 12;
    if (m) {
      var wn = String(m).trim().split(/\s+/).filter(Boolean).length;
      if (wn) n = wn;
    }
    return { n: n, bits: ENT_BITS[n] || 128 };
  }
  ...
  function entropyBarHtml(phrase) {
    var x = entropyPhraseBits(phrase);
    var checksumFail = phrase == null && mem.bip39Ok === false;
    if (checksumFail) {
      return (
    ...
    return (
      ...
      "-word list is " +
      x.bits +
      " bits of BIP-39 entropy (checksum is extra, in the last word). Practice only.</span>" +
      "</p>"
    );
  }
  ...
  function entropyMeterHtml() {
    if (!mem.mnemonic) {
      return '<div class="v2-os-ent v2-os-ent-empty" id="v2OsEnt" hidden></div>';
    }
    return '<div class="v2-os-ent" id="v2OsEnt">' + lockHtml("os") + entropyBarHtml() + "</div>";
  }

  function uc1EntropyAfterHtml(phrase) {
    var p = arguments.length ? phrase : mem.mnemonic;
    if (!p) {
      return '<div class="v2-uc1-after" id="v2Uc1After" hidden></div>';
    }
    return (
      '<div class="v2-uc1-after" id="v2Uc1After">' +
      lockHtml("os", p) +
      '<div id="v2EntropyClass">' +
      entropyTeachHtml() +
      "</div>" +
      entropyBarHtml(p) +
      "</div>"
  ...
  function faceClusterHtml(src, cap, teachTitle, teachLines, figId, teachId) {
    return (
      '<div class="v2-uc1-after v2-face-after" id="' +
      (figId || "v2FaceAfter") +
      '">' +
      ...
      teachBox(teachTitle, classLines(teachLines[0], teachLines[1], teachLines[2]), teachId || (figId ? figId + "Teach" : "v2FaceTeach")) +
      "</div></div>"
    );
  }
  ...
  function replaceOsEntropy(phrase) {
    var after = $("v2Uc1After");
    if (after) {
      after.outerHTML = arguments.length ? uc1EntropyAfterHtml(phrase) : uc1EntropyAfterHtml();
      return;
    }
    var wrap = $("v2OsEnt");
  ...
  function jobQuizzes(id) {
    var m = {
      ...
      28: [
        { q: "What is a UTXO here?", opts: [qOk("A leftover piece from an earlier payment. Your total is a pile of pieces.", "Correct. Not one bank balance."), qBad("The twelve recovery words.", "Wrong. Words open the vault. Pieces are coins on the chain.")] },
        { q: "You spend two leftover pieces in one transaction. What can an outsider guess?", opts: [qOk("Those two pieces often belong to one owner.", "Correct. That guess is the common-input heuristic."), qBad("Nothing. Combining hides everything.", "Wrong. Combining is the leak.")] },
        { q: "Why do joint spends use many equal outputs?", opts: [qOk("So an outsider cannot tell which output is yours. The chain still lists every line.", "Correct. Obfuscated is not invisible."), qBad("So bitcoin disappears from the chain.", "Wrong. Every input and output is still public.")] },
        { q: "Does a joint spend replace a paper backup?", opts: [qOk("No. Privacy sits on top of keys you already control.", "Correct."), qBad("Yes. After mixing you can throw the words away.", "Wrong.")] }
      ],
      29: [
        { q: "Same twelve words, empty extra vs extra “open”. What is true?", opts: [qOk("Two different receive addresses. Both can hold coins.", "Correct. The extra secret is not a PIN on one wallet."), qBad("Same address. The extra secret is only a screen lock.", "Wrong. That is UC3’s job too: empty vs extra are two vaults.")] },
        { q: "A wipe PIN erases the hardware. Where are the coins?", opts: [qOk("Still on the bitcoin chain. Words + the right extra open them on a new device.", "Correct. The box was never the vault."), qBad("Gone forever. Wipe deletes bitcoin.", "Wrong. Wipe deletes the chip’s copy, not the chain.")] },
        { q: "Can Ledger use two PINs for two extras on the same words?", opts: [qOk("Yes, as a device trick: each PIN can unlock a different extra. This lab does not set PINs.", "Correct. Menus change; the job is two vaults."), qBad("No. One device can only ever open one extra.", "Wrong.")] },
        { q: "Is this track what to do if someone threatens you?", opts: [qOk("No. Objects only. Not legal or personal-safety advice.", "Correct."), qBad("Yes. Follow these pads under threat.", "Wrong.")] }
      ],
      30: [
      ...
      return pad(
        ...
        teachBox(
          ...
          classLines(
            ...
            "If you type the paper copy and get the same address, the copy works.",
    ...
    return pad(
      ...
      teachBox(
        ...
        classLines(
          ...
          "If that address matches the card, the paper copy is good.",
  ...
  function uc17(step) {
    var picked = mem.tier || {};
    var placed =
      picked.coffee === "phone" && picked.mid === "hww" && picked.large === "mofn";
    function binBtn(amt, id, label) {
      var on = picked[amt] === id;
      return (
    ...
    function row(amt, label, btc) {
      return (
        '<div class="v2-tier-row" data-amt="' +
        amt +
        '" data-btc="' +
        btc +
        '"><strong class="v2-tier-label">' +
        label +
        ' · <span class="v2-tier-btc" id="v2TierBtc-' +
        amt +
        '">' +
        btc +
        " BTC</span></strong>" +
        '<div class="v2-tier-bins">' +
        binBtn(amt, "exchange", "Exchange") +
        binBtn(amt, "phone", "Phone") +
        binBtn(amt, "hww", "One HWW") +
        binBtn(amt, "mofn", "2-of-3") +
        "</div>" +
        '<div class="v2-drain v2-hidden" id="v2TierDrainWrap-' +
        amt +
        '"><span class="v2-drain-bar" id="v2TierDrain-' +
        amt +
    ...
    var teach = teachBox(
      ...
      classLines(
        ...
        "Exchange is they-hold. Wrong for every row. The number stays — they still have it.",
    ...
    var rows =
      ...
      (placed
        ...
        : "Tap one object per row. Need Daily → Phone, Mid → One HWW, Large → 2-of-3.") +
      "</p>";
    if (step === 0) {
      return pad(
        ...
        teach +
        rows +
        pauseBtn("I placed daily, mid, and large", !placed)
      );
    }
    return pad(
      "<h2>Place amounts</h2>" +
      ...
      return pad(
        ...
        teachBox(
          ...
          classLines(
            ...
            "Heirs fail because a piece is missing and nobody is there to name it.",
      ...
      return pad(
        ...
        teachBox(
          ...
          classLines(
            ...
            "If one envelope holds the words and the extra secret, one fire or one leak takes the whole vault.",
      ...
      return pad(
        ...
        teachBox(
          ...
          classLines(
            ...
            "You already proved you can restore. Now prove someone else can, with what you actually left.",
      ...
      return pad(
        ...
        teachBox(
          ...
          classLines(
            ...
            "Scammers copy the start and end and change the middle.",
      ...
      return pad(
        ...
        teachBox(
          ...
          classLines(
            ...
            "About <strong>one confirmation ≈ 10 minutes</strong> here (real blocks vary). Bigger amounts: wait more.",
    ...
    return pad(
      ...
      teachBox(
        ...
        classLines(
          ...
          "If you did not ask for it, leave it.",
  ...
  function uc21(step) {
    if (step === 3) return quizBank(jobQuizzes(21));
    if (step >= 4) return finishHtml(21);
    var c = coMem();
    if (step === 0) {
      return pad(
        ...
        "</p>" +
        pauseBtn("Next: steal vs freeze vs sign", c.policy !== "you2")
      );
    }
    if (step === 1) {
      var ok = c.steal === "no" && c.freeze === "no" && c.help === "yes";
      return pad(
  ...
  function uc22(step) {
    if (step === 0) {
      ...
      return pad(
        ...
        faceClusterHtml(
          "assets/uc22-face-firmware-hash.svg",
          "Device vs maker site",
          "Classroom — what “firmware hash checked” means",
          [
            "The device screen (or the official app talking to that screen) shows a version and a fingerprint.",
            "Compare that fingerprint to the one on the maker’s own website. Not a chat link, not a random USB.",
            "If they do not match, stop. Do not create a seed. Do not fund it."
          ],
          "v2FwFace",
          "v2FwTeach"
        ) +
    ...
    return pad(
      ...
      '<div class="v2-vault-col" id="v2VaultLaptop"><h3>Notes on the laptop</h3><p class="v2-btc-num" id="v2VaultLaptopAmt">0.184 BTC</p>' +
      '<div class="v2-drain" id="v2VaultLaptopDrainWrap" hidden><span class="v2-drain-bar" id="v2VaultLaptopDrain" style="width:100%"></span></div>' +
      "<p>Those words already lived here. This is a hot wallet, even if you later type the same words into a device.</p></div>" +
      ...
      return pad(
        ...
        teachBox(
          ...
          classLines(
            ...
            "BIP-352 is the rulebook for a code that makes a <em>new</em> mailbox each send. " +
            inlineI(
              "BIP-352",
      ...
      return pad(
        ...
        teachBox(
          ...
          classLines(
            ...
            "Here we use a practice mix so you can see the idea: same code, two different outputs.",
    ...
    return pad(
      ...
      teachBox(
        ...
        classLines(
          ...
          "A normal viewing key (zpub) is different: it lists a row of receive addresses. If leaked, that list is a camera.",
    ...
    return pad(
      ...
      teachBox(
        ...
        classLines(
          ...
          "If you opt in, use the Network room. If a lookup fails, show unknown — never a fake zero.",
      ...
      return pad(
        ...
        teachBox(
          ...
          classLines(
            ...
            "If you spend two leftovers in one payment, outsiders often guess they have the same owner.",
      ...
      return pad(
        ...
        teachBox(
          ...
          classLines(
            ...
            "That 0.18 is a sum of two pieces, not one coin.",
      ...
      return pad(
        ...
        teachBox(
          ...
          classLines(
            ...
            "It comes back to a change address (another folder on the same words).",
  ...
  function cjMem() {
    if (!mem.cj) mem.cj = { cafe: false, salary: false, combined: false, separate: false, join: false };
    return mem.cj;
  }

  function uc28(step) {
    if (step === 3) return quizBank(jobQuizzes(28));
    if (step >= 4) return finishHtml(28);
    var cj = cjMem();
    if (step === 0) {
      var both = !!(cj.cafe && cj.salary);
      return pad(
        "<h2>Leftover pieces, not a bank number</h2>" +
        doDont(
        ...
        teachBox(
          ...
          classLines(
            ...
            "Cafe 0.05 and salary 0.13 are two pieces on two addresses. The wallet may show 0.18 — a sum, not one coin.",
            "Each leftover is a UTXO. " +
            inlineI(
              "UTXO",
              "Unspent Transaction Output: a leftover piece that has not been spent yet. Spending it consumes the whole piece. Leftover value comes back as a new piece (change)."
            )
          ),
          "v2Uc28Teach"
        ) +
        ...
        '<button type="button" class="v2-metal-card' +
        (cj.cafe ? " is-on" : "") +
        '" data-cj-piece="cafe"><strong>0.05 tBTC</strong><span>Cafe leftover · one piece</span></button>' +
        '<button type="button" class="v2-metal-card' +
        (cj.salary ? " is-on" : "") +
        '" data-cj-piece="salary"><strong>0.13 tBTC</strong><span>Salary leftover · another piece</span></button>' +
        "</div>" +
        '<p id="v2CjPieceOut" class="control-help">' +
        (both ? "Two pieces. Next you will spend them." : "Tap each leftover so you see they are not one coin.") +
        "</p>" +
        pauseBtn("Next: combining", !both)
      );
    }
    if (step === 1) {
      return pad(
        "<h2>Combining two pieces looks like one owner</h2>" +
        doDont(
        ...
        teachBox(
          ...
          classLines(
            ...
            "If cafe 0.05 and salary 0.13 are both inputs, the guess is same person — the common-input heuristic. " +
              inlineI(
                "common-input heuristic",
                "A detective shortcut: pieces spent together in one payment often have one owner. It is a guess, not a proof. It is how reuse of history leaks."
              ),
        ...
        "</div>" +
        '<pre class="out" id="v2CjTx">' +
        (cj.combined
          ? "Classroom tx (not broadcast)\nIN  0.05  cafe leftover\nIN  0.13  salary leftover\nOUT 0.04  merchant\nOUT 0.14  change\nGuess: one owner. Combining linked the two stories."
          : cj.separate
            ? "Classroom tx (not broadcast)\nIN  0.05  cafe leftover\nOUT 0.04  merchant\nOUT 0.01  change\nSalary 0.13 is unspent. The merchant payment does not point at the salary piece."
            : "Build a classroom spend. It never goes to the network.") +
        "</pre>" +
        '<p id="v2CjTxMsg" class="control-help">' +
        (cj.combined && cj.separate
          ? "You saw the leak, then the quieter spend. Next: equal outputs."
          : cj.combined
            ? "That leak is the lesson. Now spend only one piece."
            : "Tap combine first so you see the guess. Then spend only one piece.") +
        "</p>" +
        pauseBtn("Next: why equal outputs", !(cj.combined && cj.separate))
      );
    }
    return pad(
      "<h2>Equal outputs obfuscate who paid</h2>" +
      doDont(
      ...
      teachBox(
        ...
        classLines(
          ...
          "The chain still lists every line. The picture is muddy, not deleted.",
      ...
      "</div>" +
      '<pre class="out" id="v2CjJoin">' +
      (cj.join
        ? "Classroom joint spend (idea only, no brand)\n5 inputs of 0.05 from 5 people\n5 outputs of 0.05\nWhich output is yours? Guess 1 of 5.\nEvery line is still public. Obfuscated ≠ invisible. Not a backup."
        : "Tap the joint spend. The other button is a trap.") +
      "</pre>" +
      pauseBtn("Mixing is not custody", !cj.join)
    );
  }

  async function duressPack() {
    await ensurePhrase();
    mem.duress = mem.duress || {};
    if (mem.duress.src === mem.mnemonic && mem.duress.empty && mem.duress.extra) return mem.duress;
    var empty = await BIP39Lab.deriveAddresses(mem.mnemonic, "", { network: "test", count: 1 });
    var extra = await BIP39Lab.deriveAddresses(mem.mnemonic, "open", { network: "test", count: 1 });
    mem.duress.src = mem.mnemonic;
    mem.duress.empty = (empty.rows && empty.rows[0] && empty.rows[0].bip84_p2wpkh) || "";
    mem.duress.extra = (extra.rows && extra.rows[0] && extra.rows[0].bip84_p2wpkh) || "";
    return mem.duress;
  }

  async function uc29(step) {
    if (step === 3) return quizBank(jobQuizzes(29));
    if (step >= 4) return finishHtml(29);
    var d = await duressPack();
    if (step === 0) {
      return pad(
        "<h2>Same words, two vaults</h2>" +
        doDont(
        ...
        teachBox(
          ...
          classLines(
            ...
            "This pad " +
              inlineI(
                "funds both",
                "Teaching amounts only. 0.01 tBTC on the empty extra, 0.50 tBTC on extra “open”. Not real coins. Not a lookup on the live chain."
              ) +
              " so you see both mailboxes can hold coins.",
        ...
        "<tr><th>Extra secret</th><th>Receive (test)</th><th>Practice balance</th></tr>" +
        "<tr><td>None (empty)</td><td><code id=\"v2DrEmpty\">" +
        (d.empty || "—") +
        "</code></td><td id=\"v2DrBalEmpty\">" +
        (d.funded ? "0.01 tBTC (practice)" : "unknown — not 0") +
        "</td></tr>" +
        "<tr><td>open (practice extra)</td><td><code id=\"v2DrExtra\">" +
        (d.extra || "—") +
        "</code></td><td id=\"v2DrBalExtra\">" +
        (d.funded ? "0.50 tBTC (practice)" : "unknown — not 0") +
        "</td></tr>" +
        "</table>" +
        '<p id="v2DrFundOut" class="control-help">' +
        (d.funded
          ? "Both vaults have practice coins. The strings must differ. Do not send real bitcoin here."
          : "The two strings must differ. Then fund both (practice).") +
        "</p>" +
        '<button type="button" class="btn" id="v2DrFund">Fund both vaults (practice only)</button>' +
        pauseBtn("Next: PINs on a device", !d.funded)
      );
    }
    if (step === 1) {
      return pad(
        "<h2>Two PINs, or a wipe PIN</h2>" +
        doDont(
        ...
        teachBox(
          ...
          classLines(
            ...
            "Menus change. The job does not: wipe removes the copy on the chip.",
        ...
        '<button type="button" class="btn secondary" data-dr-pin="wipe-kill">Wipe deletes the coins forever</button>' +
        "</div>" +
        '<p id="v2DrPinOut" class="control-help">' +
        (d.pinOk ? "Device tricks change the box, not the chain." : "Tap the two true jobs. The false one must fail.") +
        "</p>" +
        pauseBtn("Next: wipe the classroom device", !d.pinOk)
      );
    }
    return pad(
      "<h2>Wipe the box. The chain still has the coins</h2>" +
      doDont(
      ...
      teachBox(
        ...
        classLines(
          ...
          "Empty extra still opens the 0.01 practice mailbox. Extra “open” still opens the 0.50 mailbox. Same words.",
      ...
      (d.wiped
        ? "Chip empty. Chain still has both practice vaults if you have the card + extras. Not safety advice."
        : "Wipe the classroom box.") +
      "</pre>" +
      pauseBtn("Chain, not the box", !d.wiped)
    );
  }
      ...
      return pad(
        ...
        teachBox(
          ...
          classLines(
            ...
            "A child is not a backup of the parent. Lose the parent, lose the child.",
      ...
      return pad(
        ...
        teachBox(
          ...
          classLines(
            ...
            "One child is enough to see the job.",
      ...
      return pad(
        ...
        teachBox(
          ...
          classLines(
            ...
            "The twelve words alone are not enough for some vaults. Keep this line with the keys.",
      ...
      return pad(
        ...
        teachBox(
          ...
          classLines(
            ...
            "A private key, seed, or WIF in this box is a leak. Refuse it.",
      ...
      return pad(
        ...
        teachBox(
          ...
          "You will get a numbered list of English words to write on paper. That list is BIP-39. " +
            inlineI(
              "BIP-39",
              "A Bitcoin standard: secret random bits become 12 to 24 dictionary words. A checksum (a small extra check) helps catch a typo. Words are easier to copy than a giant number."
            ) +
            "<br>That card is the secret backup — like a locker combo. Later we show a receive address. " +
            inlineI(
              "address",
        ...
        '<textarea id="v2PasteMn" class="v2-share-line" rows="1" spellcheck="false" autocomplete="off"></textarea></label>' +
        '<div class="row">' +
      ...
      return pad(
        ...
        teachBox(
          ...
          classLines(
            ...
            "You would copy it onto paper in that order. Practice only. Not a funded wallet.",
            "The last word carries the checksum. " +
              inlineI(
                "checksum",
                "A small extra check packed into the last word so a typo usually fails. It is not a second password."
              )
          ),
          "v2CardWhat"
        ) +
        ...
        wordGridHtml(mem.mnemonic) +
        "</div>" +
        '<label class="check"><input type="checkbox" id="v2CardAck" ' + (mem.cardAck ? "checked" : "") + "/> I have looked at the numbered cells</label>" +
  ...
  async function uc3(step) {
      ...
      return pad(
        ...
        teachBox(
          ...
          classLines(
            ...
            "Empty A vs test B must show two receive addresses. Store the extra apart from the paper card.",
    ...
    if (step === 2) {
      return quizBank([
        {
          q: "If you forget the extra secret (passphrase) for a vault, what is true?",
          opts: [
            qOk("The twelve words alone will not open that vault. Those coins are gone.", "Correct. Same words without that extra secret open a different vault."),
            qBad("This website can reset the extra secret for you.", "Wrong. There is no reset desk. Forgotten extra secret means that vault is gone."),
            qBad("The receive addresses stay the same even if the extra secret is wrong.", "Wrong. A different extra secret makes different addresses.")
          ]
        },
        {
          q: "Same numbered card, empty extra vs a test extra — what did you see?",
          opts: [
            qOk("Two different receive addresses. That is two vaults, not a PIN on one wallet.", "Correct. The extra secret is not a PIN. It opens a different locker."),
            qBad("The same address, because the twelve words did not change.", "Wrong. Same words + a different extra make a different address."),
            qBad("A PIN that unlocks the same vault after three tries.", "Wrong. There is no retry desk. Forget the extra and that vault is gone.")
          ]
        },
        {
          q: "Where should you store the extra secret?",
          opts: [
            qOk("Apart from the paper card. Anyone with both has the vault.", "Correct. One envelope with words plus extra is the whole wallet in one leak."),
            qBad("Written on the same paper as the twelve words.", "Wrong. Then one find is the whole vault."),
            qBad("Only in this website, so you can reset it later.", "Wrong. This tab does not store your extra secret.")
          ]
        }
      ]);
    }
      ...
      return pad(
        ...
        teachBox(
          ...
          classLines(
            ...
            "A new folder is a new address, not a new recovery phrase. The numbered card stays put.",
      ...
      return pad(
        ...
        teachBox(
          ...
          classLines(
            ...
            "Leftover from a spend goes to change, not back to the receive address.",
  ...
  async function uc5(step) {
    ...
    if (step === 1) {
      ...
      return pad(
        ...
        woTypeTabsHtml() +
        faceClusterHtml(
          "assets/uc5-face-binoculars.svg",
          "Look, cannot spend",
          "Classroom — what you export",
          [
            "You export a public viewing key (and its recipe) for one folder — not the twelve words.",
            "A watch-only app can list addresses and incoming payments. It cannot spend.",
            "Never type the recovery words into that app. Pick one folder tab. What you see below is that folder only."
          ],
          "v2Wo"
        ) +
    ...
    if (step === 2) {
      return quizBank([
        {
          q: "What should a watch-only wallet receive?",
          opts: [
            qOk("A public viewing key (the long xpub or zpub string) — never the twelve recovery words.", "Correct. That key can list addresses. It cannot spend."),
            qBad("The recovery words, so the watch app can “just work”.", "Wrong. Pasting the words makes a hot wallet that can spend, not watch-only."),
            qBad("Your extra secret in the same photo as the words.", "Wrong. The extra secret spends. Watch-only only gets a public key.")
          ]
        },
        {
          q: "What can a viewing key do?",
          opts: [
            qOk("List receive addresses and incoming payments. It cannot move coins.", "Correct. Watch-only is a camera, not a spender."),
            qBad("Sign payments, because it came from the same words.", "Wrong. Signing needs the secret. The viewing key is public."),
            qBad("Reset a forgotten extra secret.", "Wrong. There is no reset desk.")
          ]
        },
        {
          q: "You need another folder (legacy vs native vs taproot). What do you export?",
          opts: [
            qOk("The viewing key (and recipe) for that folder only.", "Correct. One tab, one folder."),
            qBad("The twelve words again, into the same app.", "Wrong. Words in a watch app make a hot wallet."),
            qBad("Every folder’s private key.", "Wrong. Watch-only is public only.")
          ]
        }
      ]);
    }
  ...
  function paintMsPolicy() {
    ...
    if (zs.length !== 3) {
      pol.innerHTML =
        "<strong>Classroom — what 2-of-3 is</strong>Show all three viewing keys first. Then this box explains the spend rule. The line below is the recipe.";
    ...
    pol.innerHTML =
      ...
      classLines(
        ...
        "Each still keeps a full backup — not scraps of one phrase. The wallet also needs this recipe. Lose the recipe and you can be locked out even if the words survive.",
        "Save it with the three viewing keys in the same A–Z order. This pad does not sign."
      );
  ...
  async function uc7(step) {
    ...
    var split = !!(mem.shamirShares && mem.shamirShares.length);
    if (step === 0) {
      return pad(
        "<h2>One practice phrase</h2>" +
        doDont(
          "Generate the phrase on this pad. Look at the numbered card and the entropy cluster.",
          "Do not fund it. Do not split yet. This is not three cosigner keys (UC6)."
        ) +
        desc(
          "Shamir starts from one secret. These words are that secret. The next pad cuts them into classroom hex shares."
        ) +
        faceClusterHtml(
          "assets/uc7-face-one-secret.svg",
          "One secret",
        ...
        "</div>" +
        uc1EntropyAfterHtml(has ? mem.shamirMnemonic : "") +
        pauseBtn("Next: split these words", !has)
      );
    }
    if (step === 1) {
      return pad(
        "<h2>Split into hex shares</h2>" +
        doDont(
          "Pick a threshold. Split the same words that are still on this pad.",
          "Do not change the phrase on another screen. Do not fund it. This is not three cosigner keys."
        ) +
        (has
          ? '<div id="v2ShCard">' +
            wordGridHtml(mem.shamirMnemonic, "v2ShWordGrid") +
            "</div>" +
            uc1EntropyAfterHtml(mem.shamirMnemonic)
          : '<p class="msg-bad">Generate a practice phrase on the previous pad first.</p>') +
        faceClusterHtml(
          "assets/uc7-face-shards.svg",
          "One secret, pieces",
          "Classroom — what this split is",
          split
            ? [
                "Those shares were built from the words on this pad.",
                "Hex pieces are the result.",
                "Next pad: paste M lines and try, or combine any M."
              ]
            : [
                "The grid is still the secret.",
                "This blue box stays the teaching story.",
                "Share lines appear in the orange result box."
              ],
          "v2ShFace",
          "v2ShStory"
        ) +
        ...
        (split
          ? (mem.shamirShares || []).map(function (s) {
              return window.ShamirLab ? ShamirLab.encodeShare(s) : "";
            }).join("\n")
          : has
            ? "The grid is the secret. Click Split these words into shares."
            : "Generate a practice phrase first.") +
        "</pre>" +
        pauseBtn("Next: try M shares", !split)
      );
    }
    if (step === 2) {
      return pad(
        "<h2>Try M shares</h2>" +
        doDont(
          "Paste any M of the printed shares. Try these M shares, or Combine any M.",
          "Do not treat one hex line as a wallet. Do not fund it."
        ) +
        faceClusterHtml(
          "assets/uc7-face-any-m.svg",
          "Any M good pieces",
          "Classroom — any M rebuild",
          [
            "N pieces exist. Any M of them rebuild the same words.",
            "A wrong extra line does not cancel two good ones.",
            "Combining is recovery. A share cannot sign a spend."
          ],
          "v2ShTryFace",
          "v2ShTryTeach"
        ) +
        ...
        (did
          ...
          : split
            ...
            : "Split on the previous pad first.") +
        "</pre>" +
        ...
        (split && window.ShamirLab
          ? (mem.shamirShares || []).map(function (s) {
              return ShamirLab.encodeShare(s);
            }).join("\n")
          : "") +
        "</textarea></label>" +
    ...
    if (step === 3) {
      ...
      return pad(
        ...
        faceClusterHtml(
          "assets/uc7-face-slip39-papers.svg",
          "Three paper lists",
      ...
      return pad(
        ...
        teachBox(
          ...
          classLines(
            ...
            "It can still list every receive address that account will ever use — like a camera on the mailboxes.",
      ...
      return pad(
        ...
        teachBox(
          ...
          classLines(
            ...
            "One receive address is one mailbox. The viewing key can list every future mailbox that account will use.",
      ...
      return pad(
        ...
        teachBox(
          ...
          classLines(
            ...
            "Anyone who has the viewing key can write this list. They do not need the twelve words.",
      ...
      return pad(
        ...
        teachBox(
          ...
          classLines(
            ...
            "When you spend, the fee comes from the leftover coins you use — not from a separate “fee account.”",
        ...
        teachBox(
          ...
          classLines(
            ...
            "0 sats with status ok means the address is empty. That is a real zero.",
  ...
  async function uc12(step) {
    ...
    if (step === 0) {
      return pad(
        ...
        faceClusterHtml(
          "assets/uc12-face-phone-leak.svg",
          "Phone is online",
          "Classroom — hot on a phone",
          [
            "This pad puts the practice phrase on a phone that talks to the internet.",
            "Seed, private key, and public key all sit on that same device.",
            "Malware on the phone can copy the words. A hardware signer is the next pad."
          ],
          "v2Uc12"
        ) +
        '<div id="v2PlaceCard">' +
    ...
    if (step === 2) {
      return quizBank([
        {
          q: "Same recovery words on a phone app versus a hardware signer — what is true?",
          opts: [
            qOk("Same words, different home. A phone is hot. Hardware should keep the keys on the device.", "Correct. Where the keys live is the lesson. A USB cable is not an air-gap."),
            qBad("Plugging USB into a laptop automatically makes an air-gap.", "Wrong. A cable to an online computer is not an air-gap."),
            qBad("The safe setup is to type the recovery words into the computer.", "Wrong. Typing the words into a computer still kills the vault.")
          ]
        },
        {
          q: "You already typed the words into a laptop. What is that vault?",
          opts: [
            qOk("Still a software vault. Putting the same words on hardware later does not un-steal them.", "Correct. A leaked software seed stays leaked."),
            qBad("Cold, because you bought a hardware box afterward.", "Wrong. The laptop copy is still hot."),
            qBad("Fine if you delete the app.", "Wrong. The computer already saw the words.")
          ]
        },
        {
          q: "What is an air-gap here?",
          opts: [
            qOk("Keys that never sit on an online computer. A USB cable to a laptop is not that.", "Correct."),
            qBad("Any device with the word hardware on the box.", "Wrong. Brand is not the split."),
            qBad("This website after you click Clear secrets.", "Wrong. This tab is still a computer on a network.")
          ]
        }
      ]);
    }
  ...
  async function uc13(step) {
    ...
    if (step === 0) {
      return pad(
        ...
        faceClusterHtml(
          "assets/uc13-face-hot-cold.svg",
          "Hot vs cold",
          "Classroom — the split is the wire",
          [
            "Hot means the keys sit on a machine that talks to the internet.",
            "Cold means they do not. Brand on the box is not the split.",
            "Sort the four objects. A hardware app on a phone is still hot."
          ],
          "v2SortFace"
        ) +
    ...
    if (step === 2) {
      return quizBank([
        {
          q: "Hot versus cold means:",
          opts: [
            qOk("Whether your keys live on a machine that talks to the internet. The brand on the box is not the split.", "Correct. Daily spend can be hot. Savings stay cold or watch-only."),
            qBad("Whatever the box says “hardware”, even if you typed the words into a laptop.", "Wrong. Keys on an online machine are hot, whatever the box says."),
            qBad("The same as a company holding the keys versus you holding them.", "Wrong. An exchange is them-holding. Hot versus cold is about whether your keys are online.")
          ]
        },
        {
          q: "Hardware-wallet software running on a phone is:",
          opts: [
            qOk("Hot. The keys sit on a phone that goes online.", "Correct. Brand on the store listing is not cold."),
            qBad("Cold, because the listing said hardware.", "Wrong. Keys on a phone are hot."),
            qBad("Watch-only, because it cannot spend.", "Wrong. If the seed is on the phone, it can spend.")
          ]
        },
        {
          q: "Where should large savings live?",
          opts: [
            qOk("Cold keys or watch-only. Not the same phone as coffee money.", "Correct. Different amounts, different objects."),
            qBad("All on one exchange account for simplicity.", "Wrong. Then the company holds the keys."),
            qBad("All on the daily-spend phone so you never lose it.", "Wrong. One stolen phone takes daily and savings.")
          ]
        }
      ]);
    }
  ...
  function lockHtml(kind, phrase) {
    var m = phrase != null ? phrase : mem.mnemonic;
    var checksumFail = phrase == null && mem.bip39Ok === false;
    var ratio = kind === "os" ? (m ? (checksumFail ? 0 : 1) : null) : entLockRatio();
  ...
  function applyLockTint() {
    document.querySelectorAll(".v2-lock").forEach(function (el) {
      if (el.getAttribute("data-lock-fixed") === "1") return;
  ...
  async function uc15(step) {
    if (step === 0) {
      return pad(
        ...
        faceClusterHtml(
          "assets/uc15-face-two-vaults.svg",
          "Same words, extra door",
          "Classroom — pad first",
          [
            "First the dice or coin pad (how mixed the rolls are), then how many BIP-39 words, then an optional extra secret.",
            "A long extra secret does not fix a pad that is TOO LOW.",
            "Watch the lock and the bit count below. Those tell you if the pad is enough."
          ],
          "v2Uc15"
        ) +
    ...
    if (step === 2) {
      return quizBank([
        {
          q: "A long extra secret on a TOO LOW dice pad means:",
          opts: [
            qOk("The dice pad is still too weak. The extra secret does not replace rolling more dice.", "Correct. Twelve words want about 128 bits of pad. Twenty-four want about 256. The extra secret is another vault secret, not extra dice."),
            qBad("Add the extra-secret estimate to the pad and you now have a 24-word wallet.", "Wrong. A short pad stays a short pad. You do not add the two estimates together."),
            qBad("The computer always stretches the pad to 512 bits, so few rolls are fine.", "Wrong. That 512 number is the size of the output file, not how random your few rolls were.")
          ]
        },
        {
          q: "Which number is the source of the words?",
          opts: [
            qOk("The dice or coin pad. Roll until it meets 12-word (~128) or 24-word (~256).", "Correct. The pad is first."),
            qBad("The extra secret length. Longer extra means you can stop rolling.", "Wrong. Extra secret is another vault, not extra dice."),
            qBad("The lock picture, even if the pad is still TOO LOW.", "Wrong. The lock follows the pad bits.")
          ]
        },
        {
          q: "Same words plus a different extra secret make:",
          opts: [
            qOk("A different vault. Forget the extra and that vault is gone.", "Correct. Same lesson as the hidden 25th word."),
            qBad("The same vault with a PIN.", "Wrong. It is not a PIN on one wallet."),
            qBad("A stronger pad so TOO LOW becomes OK.", "Wrong. Extra secret does not fix a short pad.")
          ]
        }
      ]);
    }
  ...
  function mnemonicHelpHtml(inline) {
    ...
    return (
      ...
      "Practice only — not a funded wallet. " +
      '<a href="https://github.com/bitcoin/bips/blob/master/bip-0039.mediawiki" target="_blank" rel="noopener noreferrer">BIP-39 spec</a>.' +
      "</span></span></span></" +
      tag +
      ">"
    );
  ...
  function generateExplainerHtml() {
    return (
      ...
      "Practice only — not real money. Nothing is sent to the internet." +
      "</p>"
    );
  }
  ...
  /* UC design: web/v2/uc-design.md — 3-line classroom, . (i) after the term, entropy after a real list. */
  function classLines(a, b, c) {
    return [a, b, c].filter(function (s) { return !!s; }).join("<br>");
  }
  ...
  function wireStep() {
    ...
    function drainToZero(bar, amt, labelZero, onDone, startAmt) {
      if (typeof labelZero === "function") {
        startAmt = onDone;
        onDone = labelZero;
        labelZero = true;
      }
      if (mem.drainTimer) clearInterval(mem.drainTimer);
      var start = startAmt != null && !isNaN(Number(startAmt)) ? Number(startAmt) : 0.184;
      var pct = 100;
      var btc = start;
      var stepBtc = start / 5;
      if (bar) {
        bar.classList.add("is-loss");
        bar.style.width = "100%";
      }
      mem.drainTimer = setInterval(function () {
        pct -= 20;
        btc = Math.max(0, +(btc - stepBtc).toFixed(3));
        if (pct <= 0) {
          pct = 0;
          btc = 0;
          clearInterval(mem.drainTimer);
        ...
        if (amt) amt.textContent = btc.toFixed(3) + " BTC";
      }, 160);
    }
    if ($("v2PlacePhone")) {
    ...
    if (shp) {
      shp.addEventListener("click", async function () {
        ...
        replaceOsEntropy(mem.shamirMnemonic);
        var splitBtn = $("v2Sh");
        ...
        if (story0) {
          story0.innerHTML =
            ...
            classLines(
              "These " + wc + " words are the secret on this pad.",
              "Split will cut them into hex pieces.",
              "The result box will hold share lines only."
            );
        }
        var sout = $("v2ShOut");
        if (sout) {
        ...
        if (pause) pause.disabled = !mem.shamirMnemonic;
      });
    }
    var shmn = $("v2ShMN");
    if (shmn) {
    ...
    if (sh) sh.addEventListener("click", function () {
      ...
      if (pause) pause.disabled = !mem.shamirShares;
    });
    var shc = $("v2ShCombine");
    ...
    function inspectV2Psbt(raw, story) {
      ...
      if (teach) {
        teach.innerHTML =
          ...
          classLines(
            "A PSBT is an unfinished bitcoin send you can pass around without the seed.",
            "Hot software can build it; a cold device or a co-signer adds a signature later. Nobody pastes the twelve words.",
            "Inspect here. Sign on a device you trust. Broadcast from a machine you choose. This tab stops at inspect."
          );
      }
      if (!out) return;
      if (r.status !== "ok") {
    ...
    document.querySelectorAll("[data-amt] [data-bin]").forEach(function (btn) {
      btn.addEventListener("click", function () {
        ...
        var out = $("v2TierOut");
        var btcEl = $("v2TierBtc-" + amt);
        var drainWrap = $("v2TierDrainWrap-" + amt);
        var drainBar = $("v2TierDrain-" + amt);
        var startBtc = parseFloat(row.getAttribute("data-btc") || "0");
        row.classList.remove("is-frozen");
        if (drainWrap) drainWrap.classList.add("v2-hidden");
        if (btcEl && !isNaN(startBtc)) btcEl.textContent = startBtc + " BTC";
        if (out) out.className = "control-help";
        if (bin === "exchange") {
          row.classList.add("is-frozen");
          if (out) {
            out.className = "control-help msg-bad";
            out.textContent =
              "Wrong. Exchange is they-hold. The number stays — they still have it. Pick Phone / One HWW / 2-of-3 on this row. Need Daily → Phone, Mid → One HWW, Large → 2-of-3.";
          }
          if (pause) pause.disabled = true;
          return;
        }
        if ((amt === "mid" || amt === "large") && bin === "phone") {
          if (drainWrap) drainWrap.classList.remove("v2-hidden");
          if (out) {
            out.className = "control-help msg-bad";
            out.textContent =
              "Wrong. Phone is only for daily coffee. One stolen phone takes this stack. Mid → One HWW. Large → 2-of-3.";
          }
          if (pause) pause.disabled = true;
          drainToZero(drainBar, btcEl, true, function () {
            if (btcEl) btcEl.textContent = "0.000 BTC stolen";
          }, startBtc);
          return;
        }
        if (amt === "coffee" && bin === "mofn") {
          if (out) {
            out.className = "control-help msg-bad";
            out.textContent =
              "Wrong. 2-of-3 is for the large stack, not coffee. Daily → Phone.";
          }
          if (pause) pause.disabled = true;
          return;
        }
        ...
        if (out) {
          out.className = "control-help" + (ok ? " msg-ok" : "");
          out.textContent = ok
            ? "Placed: 0.001 phone · 0.184 hardware · 2.0 2-of-3. Continue is unlocked."
            : "Keep placing. Need Daily → Phone, Mid → One HWW, Large → 2-of-3.";
        }
        if (pause) pause.disabled = !ok;
      });
    });
    ...
    document.querySelectorAll("[data-cj-piece]").forEach(function (btn) {
      btn.addEventListener("click", function () {
        var k = btn.getAttribute("data-cj-piece") || "";
        var cj = cjMem();
        if (k === "cafe") cj.cafe = true;
        if (k === "salary") cj.salary = true;
        btn.classList.add("is-on");
        var both = !!(cj.cafe && cj.salary);
        var o = $("v2CjPieceOut");
        if (o) o.textContent = both ? "Two pieces. Next you will spend them." : "Tap the other leftover too.";
        if (pause) pause.disabled = !both;
      });
    });
    document.querySelectorAll("[data-cj-spend]").forEach(function (btn) {
      btn.addEventListener("click", function () {
        var k = btn.getAttribute("data-cj-spend") || "";
        var cj = cjMem();
        var tx = $("v2CjTx");
        var msg = $("v2CjTxMsg");
        if (k === "both") {
          ...
          if (msg) {
            msg.className = "msg-bad";
            msg.textContent = cj.separate
              ? "You already saw the quieter spend. Combining was the leak."
              : "That leak is the lesson. Now spend only one piece.";
          }
        } else {
          if (!cj.combined) {
            if (msg) {
              msg.className = "msg-bad";
              msg.textContent = "Tap combine first so you see the guess. Then spend only one piece.";
            }
            if (pause) pause.disabled = true;
            return;
          }
          ...
          if (msg) {
            msg.className = "msg-ok";
            msg.textContent = "You saw the leak, then the quieter spend. Next: equal outputs.";
          }
        }
        if (pause) pause.disabled = !(cj.combined && cj.separate);
      });
    });
    document.querySelectorAll("[data-cj-join]").forEach(function (btn) {
      btn.addEventListener("click", function () {
        var k = btn.getAttribute("data-cj-join") || "";
        var cj = cjMem();
        var box = $("v2CjJoin");
        if (k === "hide") {
          cj.join = false;
          if (box) {
            paintTone(box, "bad");
            box.textContent = "Fail. Every input and output stays on the chain. Obfuscated is not invisible.";
          }
          if (pause) pause.disabled = true;
          return;
        }
    ...
    document.querySelectorAll("[data-co-pol]").forEach(function (btn) {
      btn.addEventListener("click", function () {
        var k = btn.getAttribute("data-co-pol") || "";
        var c = coMem();
        var o = $("v2CoPolOut");
        var ok = k === "you2";
        c.policy = ok ? "you2" : k;
        if (o) {
          o.className = ok ? "msg-ok" : "msg-bad";
          o.textContent = ok
            ? "2-of-3. Your two keys are enough to spend."
            : k === "they1"
              ? "Fail. One company key cannot spend. That would be steal."
              : "Fail. Needing all three is 3-of-3. This product is 2-of-3.";
        }
        if (pause) pause.disabled = !ok;
      });
    });
    document.querySelectorAll("[data-co-q]").forEach(function (btn) {
      btn.addEventListener("click", function () {
        var q = btn.getAttribute("data-co-q") || "";
        var a = btn.getAttribute("data-co-a") || "";
        var c = coMem();
        var want = { steal: "no", freeze: "no", help: "yes" };
        var right = want[q] === a;
        c[q] = a;
        var o = $("v2CoThreatOut");
        var all = c.steal === "no" && c.freeze === "no" && c.help === "yes";
        if (!right) {
          if (o) {
            o.className = "msg-bad";
            o.textContent =
              q === "steal"
                ? "Fail. One key cannot steal."
                : q === "freeze"
                  ? "Fail. You already have two keys. They are optional for this spend."
                  : "Fail. If you need their signature, they can refuse. That delays recovery. It is not theft.";
          }
          if (pause) pause.disabled = true;
          return;
        }
        if (o) {
          ...
          o.textContent = all
            ? "Steal: no. Freeze while you have two keys: no. Refuse recovery help: yes."
            : "Correct so far. Finish all three.";
        }
        if (pause) pause.disabled = !all;
      });
    });
    ...
    document.querySelectorAll("[data-laptop-seed]").forEach(function (btn) {
      btn.addEventListener("click", function () {
        ...
        var lapAmt = $("v2VaultLaptopAmt");
        var lapWrap = $("v2VaultLaptopDrainWrap");
        var lapBar = $("v2VaultLaptopDrain");
        if (stay) {
          if (mem.drainTimer) {
            clearInterval(mem.drainTimer);
            mem.drainTimer = 0;
          }
          if (lapWrap) lapWrap.hidden = true;
          if (lapBar) {
            lapBar.classList.remove("is-loss");
            lapBar.style.width = "100%";
          }
          if (lapAmt) lapAmt.textContent = "0.184 BTC";
        } else {
          if (lapWrap) lapWrap.hidden = false;
          drainToZero(lapBar, lapAmt, true, function () {
            if (lapAmt) lapAmt.textContent = "0.000 BTC stolen";
            if (hwAmt) hwAmt.textContent = "0.000 BTC stolen";
          });
        }
    ...
    if ($("v2DrFund")) {
      $("v2DrFund").addEventListener("click", function () {
        var d = mem.duress || {};
        if (!d.empty || !d.extra || d.empty === d.extra) {
          var bad = $("v2DrFundOut");
          if (bad) {
            bad.className = "msg-bad";
            bad.textContent = "The two addresses must differ. Generate practice words first.";
          }
          return;
        }
        d.funded = true;
        mem.duress = d;
        if ($("v2DrBalEmpty")) $("v2DrBalEmpty").textContent = "0.01 tBTC (practice)";
        if ($("v2DrBalExtra")) $("v2DrBalExtra").textContent = "0.50 tBTC (practice)";
        var o = $("v2DrFundOut");
        if (o) {
          o.className = "msg-ok";
          o.textContent = "Both vaults have practice coins. The strings must differ. Do not send real bitcoin here.";
        }
        if (pause) pause.disabled = false;
      });
    }
    document.querySelectorAll("[data-dr-pin]").forEach(function (btn) {
      btn.addEventListener("click", function () {
        var k = btn.getAttribute("data-dr-pin") || "";
        mem.duress = mem.duress || {};
        var o = $("v2DrPinOut");
        if (k === "wipe-kill") {
          if (o) {
            o.className = "msg-bad";
            o.textContent = "Fail. Wipe deletes the chip’s copy. The coins stay on the chain.";
          }
          if (pause) pause.disabled = true;
          return;
        }
        if (k === "ledger") mem.duress.ledger = true;
        if (k === "wipe-ok") mem.duress.wipeOk = true;
        mem.duress.pinOk = !!(mem.duress.ledger && mem.duress.wipeOk);
        if (o) {
          o.className = mem.duress.pinOk ? "msg-ok" : "";
          o.textContent = mem.duress.pinOk
            ? "Device tricks change the box, not the chain."
            : k === "ledger"
              ? "Ledger: two PINs can map to two extras. Now the Coldcard wipe job."
              : "Wipe empties the chip. Now the Ledger two-PIN job.";
        }
        if (pause) pause.disabled = !mem.duress.pinOk;
      });
    });
    if ($("v2DrWipe")) {
      $("v2DrWipe").addEventListener("click", function () {
        mem.duress = mem.duress || {};
        mem.duress.wiped = true;
        var o = $("v2DrWipeOut");
        if (o) {
          paintTone(o, "ok");
          o.textContent =

...
zoo`.split("\n"));
  ...
  function bytesToNumberLE(bytes) {
    return hexToNumber(bytesToHex(copyBytes(abytes3(bytes)).reverse()));
  }
  ...
  function weierstrass(params, extraOpts = {}) {
    ...
    class Point2 {
      ...
      static fromBytes(bytes) {
        const P = Point2.fromAffine(decodePoint(abytes4(bytes, void 0, "point")));
        P.assertValidity();
        return P;
      }
      static fromHex(hex) {
  ...
  function sortPubkeysBIP67(pubs) {
    return pubs.slice().sort((a, b) => {
      const ha = bytesToHex3(a);
      const hb = bytesToHex3(b);
  ...
  function buildMultisigFromText(partsText, m, options) {
    ...
    const scriptHex = bytesToHex3(script);
    const p2sh = p2shAddress(script);
    const p2wsh = p2wshAddress(script);
    const pubkeysHex = pubs.map(bytesToHex3);
    ...
    return {
      ...
      descriptor,
      keyIds,
      vaultMapNote: "Public vault map — not a seed. Back up this string with each key. Lose the map AND one key and you may not rebuild the vault.",
      scriptHex,
      p2sh,
      p2wsh,
